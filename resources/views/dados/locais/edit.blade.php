...
<div class="container-fluid" style="visibility:visible;position:absolute;background-color:#1d1248;height:80%;width:30%;margin-left:25%;top:0%;margin-top:2%;box-shadow: 0 8px 16px 0 rgba(0,0,0,0.2);" id="editar">
  <div id="mapedit" style="border-radius: 15px;margin-top:8%;margin-bottom:2%;margin-left:4%;margin-right:4%;"></div><br>
  <form action="/locais/{{$only->id}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('put')
    <div class="row" style="margin-right:4%;margin-left:4%;">
      <div class="col-12">
      <input type="text" class="form-control" name="nomelocal" id="nomelocal" placeholder="Nome" value="{{$only->nomelocal}}"></div>
    </div><br>

    <div class="row" style="margin-right:4%;margin-left:4%;margin-top:-2%;">
      <div class="col-6">
        <input type="text" class="form-control" name="emaillocal" id="emaillocal" placeholder="Email" style="width: 100%" value="{{$only->emaillocal}}">
      </div>
      <div class="col-6">
        <input type="text" class="form-control" name="telelocal" id="telelocal" placeholder="Telefone" style="width: 100%" value="{{$only->telelocal}}">
      </div>
    </div><br>

    <div class="row" style="margin-right:4%;margin-left:4%;margin-top:-2%;">
<div class="col-4">
  <input type="text" class="form-control" id="lat" name="lat" placeholder="Latitude" value="{{$only->Latitude}}">
</div>
<div class="col-4">
  <input type="text" class="form-control" id="lng" name="lng" placeholder="Longitude" value="{{$only->Longitude}}">
</div>
<div class="col-4">
...
<div class="col-7">
  <textarea class="form-control" name="descrilocal" id="descrilocal" placeholder="Descrição" rows="3">{{$only->descrilocal}}</textarea>
</div>
    </div><br>
