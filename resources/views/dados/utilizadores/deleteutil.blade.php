    ...
    <table class="table" style="color:#1d1248;">
        <thead>
          <tr style="background-color:#F8E08E;">
            <th scope="col" style="text-align: center;"><b>Eliminar Utilizador</b></th>
          </tr>
        </thead>
        <tbody style="background-color: white">
          <td scope="row">
            <i class="fas fa-user-slash" style="font-size: 65px;margin-left:32.5%;margin-right:32.5%;margin-top:5%;margin-bottom:5%;"></i>
            <h5 style="text-align: center;margin-bottom:10%;">Deseja mesmo eliminar "{{$only->name}}" ?</h5>
            <form action="/users/{{$only->id}}" method="POST" style="margin-left:22%;">
                @csrf
                @method('delete')
                <button type="submit" style="color:green;" class="btndelete" value="Eliminar"><i class="fas fa-check"></i></button>
                <button type="button" style="color:red;" class="btndelete" onClick="window.location.href='/users'" value="Cancelar"><i class="fas fa-ban"></i></button>
              </form>
